...
class Value(object):
    ...
    def string(self, length):
        ...
        read_len = min(_STR_MAX_LEN, len)
        char_size = self._sbvalue_object.GetType().GetPointeeType().GetByteSize()
        target = lldb.debugger.GetSelectedTarget()
        sbaddr = lldb.SBAddress(self._sbvalue_object.GetValueAsUnsigned(), target)
        src_encoding = 'utf-%d' % (char_size * 8)
        dst_encoding = 'utf-8'
        error = lldb.SBError()
        encoded_str = str(target.ReadMemory(sbaddr, read_len * char_size, error))
        if error.Fail():
          return "Failed to read memory: %s" % (error.GetCString())
        return encoded_str if src_encoding == dst_encoding else encoded_str.decode(src_encoding).encode(dst_encoding)
