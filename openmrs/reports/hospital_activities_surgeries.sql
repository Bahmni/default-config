...
-- Query 
   ...
   SELECT
   IF( prime_obs.concept_full_name = 'Major Surgery TBD', 'Major',
   ...
   SUM(CASE WHEN (prime_obs.concept_full_name = 'Operative Procedure' OR prime_obs.concept_full_name = 'Major Surgery TBD') AND person.gender = 'F' THEN 1 ELSE 0 END) AS female,
   SUM(CASE WHEN (prime_obs.concept_full_name = 'Operative Procedure' OR prime_obs.concept_full_name = 'Major Surgery TBD') AND person.gender = 'M' THEN 1 ELSE 0 END) AS male,
   ...
   WHERE prime_obs.concept_full_name  IN ('Operative Procedure', 'Major Surgery TBD')
