...
LEFT OUTER JOIN (select max(encounter_datetime) as admission_datetime, patient_id from encounter_view where encounter_type_name='ADMISSION' group by patient_id) last_admission on person.person_id = last_admission.patient_id and last_admission.admission_datetime BETWEEN '%s' and '%s'
where
