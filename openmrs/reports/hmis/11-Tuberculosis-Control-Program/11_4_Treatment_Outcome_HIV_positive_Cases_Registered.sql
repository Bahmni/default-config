...
FROM
    ...
    INNER JOIN concept_name answer_concept_fully_specified_name ON ca.answer_concept = answer_concept_fully_specified_name.concept_id
		...
		AND answer_concept_fully_specified_name.name  IN ('Yes')
    ...
    LEFT JOIN concept_name answer_concept_short_name ON ca.answer_concept = answer_concept_short_name.concept_id
        ...
        IS FALSE
    WHERE
        question_concept_name.name IN ('DRTuberculosis, HIV Infection')
            AND cd.name = 'Coded'
    ...
    INNER JOIN concept_name cn1 ON o1.concept_id = cn1.concept_id
        ...
        AND cn1.name = 'DRTuberculosis, HIV Infection'
        AND o1.voided = 0
        AND cn1.voided = 0
    ...
    WHERE
        CAST(v1.date_started AS DATE) BETWEEN DATE(DATE_SUB('#startDate#', INTERVAL 20 MONTH)) AND DATE(DATE_SUB('#endDate#', INTERVAL 20 MONTH))) first_concept ON first_concept.answer = first_answers.answer
        LEFT OUTER JOIN
    (SELECT 
        DISTINCT(o1.person_id),
            cn2.concept_id AS answer,
    ...
    INNER JOIN visit v1 ON v1.visit_id = e.visit_id
    WHERE
        CAST(v1.date_started AS DATE) BETWEEN DATE(DATE_SUB('#startDate#', INTERVAL 20 MONTH)) AND DATE(DATE_SUB('#endDate#', INTERVAL 20 MONTH)))second_concept ON 
