...
FROM
    ...
    LEFT JOIN concept_name answer_concept_short_name ON ca.answer_concept = answer_concept_short_name.concept_id
        ...
        IS FALSE
    WHERE
        question_concept_name.name IN ('Tuberculosis, HIV Infection')
            AND cd.name = 'Coded'
    ...
    INNER JOIN concept_name cn1 ON o1.concept_id = cn1.concept_id
        ...
        AND cn1.name = 'Tuberculosis, HIV Infection'
        AND o1.voided = 0
        AND cn1.voided = 0
    ...
    INNER JOIN visit v1 ON v1.visit_id = e.visit_id
        AND v1.date_stopped IS NOT NULL) first_concept ON first_concept.answer = first_answers.answer
        LEFT OUTER JOIN
