SELECT 
    second_answers.answer_name AS 'Categories',
    first_answers.answer_name AS 'Sputum Conversion ',
...
FROM
    ...
    LEFT JOIN concept_name answer_concept_short_name ON ca.answer_concept = answer_concept_short_name.concept_id
        ...
        IS FALSE
    WHERE
        question_concept_name.name IN ('TB FU-Sputum conversion result')
            AND cd.name = 'Coded'
    ...
    INNER JOIN concept_name cn1 ON o1.concept_id = cn1.concept_id
        AND cn1.concept_name_type = 'FULLY_SPECIFIED'
        AND cn1.name = 'TB FU-Sputum conversion result'
        AND o1.voided = 0
        AND cn1.voided = 0
    INNER JOIN concept_name cn2 ON o1.value_coded = cn2.concept_id
        AND cn2.concept_name_type = 'FULLY_SPECIFIED'
