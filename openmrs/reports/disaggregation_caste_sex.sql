...
RIGHT OUTER JOIN (SELECT child_concept_name FROM concept_children_view WHERE parent_concept_name = 'Caste Type' ) AS caste_list ON caste_list.child_concept_name = person_attribute.value
GROUP BY caste_list.child_concept_name) AS underweight ON underweight.caste_ethnicity = cbimci.caste_ethnicity
INNER JOIN
...
RIGHT OUTER JOIN (SELECT child_concept_name FROM concept_children_view WHERE parent_concept_name = 'Caste Type' ) AS caste_list ON caste_list.child_concept_name = person_attribute.value
GROUP BY caste_list.child_concept_name) AS delivery ON delivery.caste_ethnicity = underweight.caste_ethnicity
INNER JOIN
-- Abortion cases
(SELECT
	caste_list.child_concept_name AS caste_ethnicity,
...
INNER JOIN coded_obs_view ON encounter.encounter_id = coded_obs_view.encounter_id
    ...
    AND coded_obs_view.value_concept_full_name IS NOT NULL
RIGHT OUTER JOIN (SELECT child_concept_name FROM concept_children_view WHERE parent_concept_name = 'Caste Type' ) AS caste_list ON caste_list.child_concept_name = person_attribute.value
GROUP BY caste_list.child_concept_name) AS hiv_cases
INNER JOIN
...
RIGHT OUTER JOIN (SELECT child_concept_name FROM concept_children_view WHERE parent_concept_name = 'Caste Type' ) AS caste_list ON caste_list.child_concept_name = person_attribute.value
GROUP BY caste_list.child_concept_name) AS gender_violence ON gender_violence.caste_ethnicity = tb_cases.caste_ethnicity;
