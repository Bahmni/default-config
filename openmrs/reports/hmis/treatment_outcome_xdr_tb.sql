...
FROM
  ...
  (SELECT
   ...
   FROM obs AS outcome_obs
     JOIN obs AS xdr_tb_obs ON outcome_obs.person_id = xdr_tb_obs.person_id AND xdr_tb_obs.voided = FALSE AND outcome_obs.voided = FALSE
     ...
     JOIN obs AS certainty_obs ON certainty_obs.person_id = xdr_tb_obs.person_id AND certainty_obs.obs_group_id = xdr_tb_obs.obs_group_id AND certainty_obs.voided = FALSE
     ...
     LEFT JOIN obs ruled_out ON ruled_out.value_coded = ruled_out_concept.concept_id
                                AND ruled_out.obs_group_id = xdr_tb_obs.obs_group_id
   WHERE ruled_out.value_coded IS NULL
  ) AS outcome
    ON outcome_concept_answer.answer_concept = outcome.value_coded AND cast(outcome.obs_datetime AS DATE) BETWEEN CAST('#startDate#' AS DATE) AND CAST('#endDate#' AS DATE)
  LEFT JOIN person p ON outcome.person_id = p.person_id
GROUP BY Outcome;
