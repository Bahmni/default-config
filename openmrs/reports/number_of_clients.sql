-- Parameters
SET @start_date = '2015-03-10';
SET @end_date = '2015-03-12';
-- Dependency 
...
SELECT client_visits.age_group AS Age_Group,
	IF(client_visits.patient_id IS NULL, 0, SUM(IF(DATE(client_visits.first_visit_date) BETWEEN @start_date AND @end_date,IF(client_visits.patient_gender = 'F', 1, 0),0))) AS New_Clients_Female,
	IF(client_visits.patient_id IS NULL, 0, SUM(IF(DATE(client_visits.first_visit_date) BETWEEN @start_date AND @end_date,IF(client_visits.patient_gender = 'M', 1, 0),0))) AS New_Clients_Male,
