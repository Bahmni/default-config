...
}]).directive('patientAppointmentsDashboards', ['$http', '$q', '$window','appService', function ($http, $q, $window, appService) {
    var link = function ($scope) {
        $scope.contentUrl = appService.configBaseUrl() + "/customDisplayControl/views/contactLists.html";
        var getContactLits = function () {
            var params = {
                q: "bahmni.sqlGet.patientContacts",
                v: "full",
                patientUuid: $scope.patient.uuid
            };
            return $http.get('/openmrs/ws/rest/v1/bahmnicore/sql', {
        ...
        $q.all([getContactLits()]).then(function (response) {
            $scope.patientContacts = response[0].data;
            $scope.patientContactsHeadings = _.keys($scope.patientContacts[0]);
        });
    };
    return {
        restrict: 'E',
...
}]).directive('patientContactsElicited', ['$http', '$q', '$window','appService', function ($http, $q, $window, appService) {
    var link = function ($scope) {
        $scope.contentUrl = appService.configBaseUrl() + "/customDisplayControl/views/contactsInformation.html";
        var getElicitedContacts = function () {
            var params = {
                q: "bahmni.sqlGet.getContactsElicited",
                v: "full",
                patientUuid: $scope.patient.uuid
            };
            return $http.get('/openmrs/ws/rest/v1/bahmnicore/sql', {
        ...
        $q.all([getElicitedContacts()]).then(function (response) {
            $scope.elicitedContacts = response[0].data;
            $scope.elicitedContactsHeadings = _.keys($scope.elicitedContacts[0]);
        });
    };
    return {
        restrict: 'E',
