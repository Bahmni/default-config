...
(SELECT ov.person_id AS patient
	FROM obs ov 
		...
		WHERE date(ov.obs_datetime) BETWEEN "#startDate#" AND "#endDate#" and ov.voided=0 and ov.value_numeric > 0) AS this_month
LEFT JOIN 
(SELECT ov1.person_id as patient
	FROM obs ov1
		...
		WHERE (DATEDIFF("#startDate#", date(ov1.obs_datetime))/30 BETWEEN 0 AND 9) and date(ov1.obs_datetime) not between "#startDate#" and "#endDate#" and ov1.voided=0 and ov1.value_numeric > 0) AS last_9_months
...
SELECT 'Patients Receiving >= 180 Iron Tablets' as "String", COUNT(DISTINCT result.patient) AS Result
	...
	FROM obs ov
	INNER JOIN concept_name con on con.concept_id = ov.concept_id and con.name='ANC, Number of IFA Tablets given' and con.concept_name_type = 'FULLY_SPECIFIED'
	WHERE date(ov.obs_datetime) BETWEEN "#startDate#" AND "#endDate#" and ov.voided=0
...
SELECT 'Patients Receiving Deworming Tablets' as "String", COUNT(DISTINCT(ov.person_id)) AS Result 
	...
	WHERE ov.value_coded = 1 and (date(ov.obs_datetime) BETWEEN "#startDate#" AND "#endDate#" and ov.voided=0)
...
SELECT 'Postpartum Patients Receiving Vitamin A Capsules' as "String", COUNT(DISTINCT(ov.person_id)) AS Result
		...
		WHERE ov.value_numeric > 0 AND (date(ov.obs_datetime) BETWEEN "#startDate#" AND "#endDate#") and ov.voided=0;
